...
function ButtonIcons({ variant }: Pick<ButtonProps, "variant">) {
  return (
    <HStack gap="16" wrap align="end">
      <Button variant={variant} aria-label="Choose language" icon>
        <EnvelopeIcon />
      </Button>
...
function Example() {
  return (
    <VStack gap="24" wrap>
      <HStack gap="24" style={{ padding: "0 12px" }}>
      ...
      <HStack gap="24" style={{ padding: "0 12px" }}>
        <ButtonIcons variant="tertiary" />
      </HStack>
      <HStack gap="24" style={{ backgroundColor: "var(--hds-colors-darker)", padding: "12px" }}>
        <ButtonIcons variant="inverted" />
      </HStack>
    </VStack>
  );
}
...
/**
 * Envelope icon 32x32 icon example
 */
const EnvelopeIcon = () => (
